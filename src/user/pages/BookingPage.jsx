      ...
      <div id="booking-content-container" className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-6">
          <form id="booking-form" onSubmit={handleSubmit} className="card p-6 space-y-5">
            ...
            {!isCartBooking && selectedEquipment && selectedEquipment.status === 'Maintenance' ? (
              <div className="p-4 bg-red-50 rounded-xl border border-red-200 text-xs text-red-900 leading-relaxed text-center">
                ⚠️ Unit <strong>{selectedEquipment.name}</strong> sedang dalam masa perawatan/kalibrasi (Maintenance) dan belum dapat disewa saat ini.
              </div>
            ) : (
              <button
                type="submit"
                disabled={
                  isCartBooking
                    ? !cart || cart.length === 0 || isCompressing
                    : !selectedEquipment || isCompressing
                }
                className={`btn-primary w-full justify-center py-3 text-sm shadow-md transition-all ${
